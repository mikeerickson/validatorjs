...
describe('Wildcard', function () {
  describe('Simple Rules ', function () {
    ...
    it('should have validation fail when  rules without values are failed and show customMessage', function () {
      var validator = new Validator({
        users: [{
          name: '',
        }]
        ...
        'required.users.*.name': 'The name is required'
      });
      expect(validator.fails()).to.be.true;
      expect(validator.passes()).to.be.false;
      expect(validator.errors.all()).to.eql({
        'users.0.name': ['The name is required'],
      })
    });
