...
describe("sometimes validation pass rules", () => {
  it("should pass when the property is passed with data", () => {
    const validator = new Validator({ firstname: "Johnny", lastname: "Appleseed" }, { firstname: "required", lastname: "required|sometimes" });
  ...
  it("should be able to register and pass async rule when the property is passed with data", function (done) {
    ...
    const validator = new Validator({ username: "test", email: "test@example.com" }, { username: "username", email: "email|sometimes" });
