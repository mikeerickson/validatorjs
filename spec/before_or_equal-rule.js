...
describe("before or equal rule", function() {
  ...
  it('should fail when comparing date value is smaller', function () {
    const validator = new Validator({ date: '1996-03-03' }, { date: 'before_or_equal:1996-03-02' });

    expect(validator.fails()).to.be.true;
    expect(validator.passes()).to.be.false;
    expect(validator.errors.first("date")).to.equal("The date must be equal or before 1996-03-02.");
  });
  ...
  it('should pass when comparing date value is equal', function () {
    const validator = new Validator({ date: '1996-03-03' }, { date: 'before_or_equal:1996-03-03' });
